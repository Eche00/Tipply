    ...
    <img src="/stars.png" alt="" className="absolute top-[-20%] right-[0%] rotate-[-43.54deg] w-[269px] h-[585px] object-cover opacity-[70%] z-10" />
     ...
     <section className="flex flex-col w-[90%] mx-auto py-[50px]">
      <div className="flex sm:flex-row flex-col sm:items-start items-center sm:justify-between justify-center">
         <motion.h2 className="  text-[40px]  font-[400] leading-[55px] capitalize bg-gradient-to-r from-[#FFFFFF] to-[#FFFFFF69] bg-clip-text text-transparent text-center" initial={{ x: -50, opacity: 0 }}
          whileInView={{ x: 1, opacity: 1 }}
          transition={{ duration: 1.5, delay: 0.3 }}  viewport={{once:true}}>Core Values</motion.h2>

        <motion.p className=" text-[#AFB3CA] text-[16px] font-[400] leading-[24px] sm:text-start text-center max-w-[400px]" initial={{ scale: 0.9, opacity: 0 }}
          whileInView={{ scale: 1, opacity: 1 }}
          transition={{ duration: 1.5, delay: 0.3 }}  viewport={{once:true}}>We believe in transparency, innovation, and community-driven growth. Our commitment is to empower developers and supporters alike with privacy, security, and seamless interactions across the Web3 ecosystem.</motion.p>
      </div>

      <section className="flex  flex-wrap gap-[48px] items-stretch justify-center py-[48px] ">
        {/* mission  */}
        <motion.div className=" bg-[#141414] p-[24px] rounded-[24px] flex flex-col gap-[16px]" initial={{ y: 40,scale:0.8, opacity: 0 }}
       ...
       {/* Discover & Tip */}
          <motion.div className="flex sm:flex-row flex-col gap-[20px] bg-[#141414] rounded-[24px] sm:h-full sm:max-h-[377px] h-[752px] overflow-hidden " initial={{ scale: 0.8, opacity: 0 }}
          whileInView={{ scale: 1, opacity: 1 }}
          transition={{ duration: 1.5, delay: 0.3 }}  viewport={{once:true}}>
